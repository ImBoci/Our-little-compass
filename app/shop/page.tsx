...
export default function ShopPage() {
  ...
  const [deleteCountdowns, setDeleteCountdowns] = useState<Record<number, number>>({});
  const pendingDeletes = useRef<Record<number, ReturnType<typeof setTimeout>>>({});
  const pendingIntervals = useRef<Record<number, ReturnType<typeof setInterval>>>({});
  ...
  const addItem = async (e?: React.FormEvent) => {
    ...
    const newItem = { id: tempId, name: input.trim(), checked: false };
    ...
    if (res.ok) {
      ...
      const resolvedItem = Array.isArray(savedItem) ? savedItem[0] : savedItem;
      setItems(prev => prev.map(i => i.id === tempId ? resolvedItem : i));
    }
  };

  const clearPendingDelete = (id: number) => {
    if (pendingDeletes.current[id]) {
      clearTimeout(pendingDeletes.current[id]);
      delete pendingDeletes.current[id];
    }
    if (pendingIntervals.current[id]) {
      clearInterval(pendingIntervals.current[id]);
      delete pendingIntervals.current[id];
    }
  ...
  const toggleItem = async (id: number, currentChecked: boolean) => {
    ...
    if (newChecked) {
      ...
      setDeleteCountdowns((prev) => ({ ...prev, [id]: 3 }));
      pendingIntervals.current[id] = setInterval(() => {
        setDeleteCountdowns((prev) => {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center text-emerald-500 animate-pulse font-serif text-xl">
        Loading list...
      </div>
    );
  }
    ...
    <div className="min-h-screen p-4 flex flex-col items-center bg-transparent">
      ...
      <div className="w-full max-w-lg bg-[var(--card-bg)] backdrop-blur-xl border border-white/40 p-6 rounded-[2rem] shadow-xl flex flex-col gap-6">
        <form onSubmit={addItem} className="flex gap-2">
          ...
          <button
            type="submit"
            className="bg-emerald-500 text-white px-4 rounded-xl hover:bg-emerald-600 transition-colors shadow-md flex items-center justify-center"
          >
            <Plus size={24} />
          </button>
        </form>
        ...
        <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-1">
          {items.length === 0 && (
            <div className="text-center text-slate-500 dark:text-slate-400 italic py-8">
              Your list is empty. Time to stock up! 🥦
            </div>
          )}
          ...
          {items.map(item => (
            <div
              ...
              className={`flex items-center justify-between p-3 rounded-xl border transition-all duration-500 ${
                item.checked
                  ? "bg-emerald-50/30 border-emerald-100/50 opacity-60 scale-[0.98]"
                  : "bg-white/40 dark:bg-slate-800/40 border-white/50 dark:border-slate-600 shadow-sm"
              }`}
            >
              <div
              ...
              >
                ...
                <span
                  className={`text-lg transition-all ${
                    item.checked ? "text-slate-400 line-through" : "text-[var(--text-color)] font-medium"
                  }`}
                >
                  {item.name}
                </span>
                {deleteCountdowns[item.id] && (
                  <span className="text-xs text-emerald-500 animate-pulse ml-2 font-medium">
                    Deleting in {deleteCountdowns[item.id]}...
                  </span>
                )}
              </div>
              ...
              {item.checked ? (
                <button
                  ...
                  className="p-2 text-emerald-500 hover:scale-110 transition-transform"
                  title="Undo"
                >
                  <Undo2 size={20} />
                </button>
        ...
        <div className="flex gap-3 pt-2 border-t border-slate-200/50 dark:border-slate-700/50">
          <button
            ...
            className="flex-1 py-3 rounded-xl font-bold text-emerald-700 dark:text-emerald-200 bg-emerald-500/10 dark:bg-emerald-500/20 hover:bg-emerald-500/20 dark:hover:bg-emerald-500/30 border border-emerald-200/60 dark:border-emerald-400/30 text-sm transition-all"
          >
            Clear Done
          </button>
          <button
